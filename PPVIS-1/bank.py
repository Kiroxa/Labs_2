...
class Bank:
    ...
    def get_all_sum(store: dict) -> int:
        ''' Return a sum of money in the bank storage '''
...
class ATM:
    ...
    class Storage:
        ...
        def cash_for_out(self) -> dict:
            ''' It is a real cash in the storage that will withdraw to clent '''
        ...
        def get_money(self, cash: int) -> bool:
            ''' Check if input cash is appropriate with ATM storage money sum and prepare the real 
                cash for out to client from his creadit card '''
